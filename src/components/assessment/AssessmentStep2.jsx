...
export default function AssessmentStep2({ clientInfo, onNext, onBack }) {
  ...
  const generateQuestions = async () => {
    setLoading(true);
    const prompt = `You are an expert business consultant conducting a diagnostic assessment for a company.
...
Generate exactly 10 diagnostic questions for this business. Questions should be progressive and cover:
...
Each question should be tailored to the company's industry and size.
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 text-center py-20">
        <div className="inline-flex items-center gap-3 bg-card border border-border rounded-2xl px-8 py-6">
          ...
          <span className="text-foreground font-medium">Generating your personalized assessment...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-6">
      ...
      <div className="bg-card border border-border rounded-2xl p-8 md:p-10">
        <h3 className="text-xl md:text-2xl font-semibold text-foreground mb-8 leading-snug">
          {q.question}
        </h3>
        ...
        {q.type === "multiple_choice" && q.options && (
          ...
          >
            {q.options.map((opt, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-4 rounded-xl border border-border hover:border-primary/30 hover:bg-primary/5 transition-all cursor-pointer"
              >
                <RadioGroupItem value={opt} id={`opt-${i}`} />
                <Label htmlFor={`opt-${i}`} className="text-sm text-foreground cursor-pointer flex-1">
                  {opt}
                </Label>
              </div>
            ))}
        ...
        {q.type === "yes_no" && (
          <div className="flex gap-4">
            <Button
              ...
              className="flex-1 h-14 text-base rounded-xl hover:border-primary/30 hover:bg-primary/5"
            >
              Yes
            </Button>
            <Button
